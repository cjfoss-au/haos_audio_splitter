...
su - audioaddon -c "ffmpeg -f pulse -i default -af 'pan=mono|c0=c0' -f wav - | cvlc --intf telnet --telnet-password leftpass --telnet-port 4212 - &"
su - audioaddon -c "ffmpeg -f pulse -i default -af 'pan=mono|c0=c1' -f wav - | cvlc --intf telnet --telnet-password rightpass --telnet-port 4213 - &"
...
echo "LADSPA_PATH set to: $LADSPA_PATH"
...
su - audioaddon -c "pactl load-module module-ladspa-sink sink_name=mono_left master=$DEFAULT_SINK plugin=mono label=mono control=1"
su - audioaddon -c "pactl load-module module-ladspa-sink sink_name=mono_right master=$DEFAULT_SINK plugin=mono label=mono control=2"
